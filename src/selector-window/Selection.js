import {selectBoxEl, topBtn, rightBtn, bottomBtn, leftBtn, toolbarEl} from './DOM.js';
...
export default class Selection {
  ...
  getSelectBoxPosition() {
    return {
      width: selectBoxEl.offsetWidth - 1,
      height: selectBoxEl.offsetHeight - 1,
      left: selectBoxEl.offsetLeft + 1,
      top: selectBoxEl.offsetTop + 1
    };
  }
  ...
  /**
   * 显示工具栏
   * @param {{top: number, left: number, width: number, height: number}} selectBoxPosition 选择框位置
   * @returns {void}
   */
  showToolbar(selectBoxPosition) {
    ...
    if (selectBoxPosition.top + selectBoxPosition.height < window.innerHeight - 34) {
      toolbarEl.style.top = `${selectBoxPosition.top + selectBoxPosition.height + 4}px`;
    }else if (selectBoxPosition.top >= 34) {
      toolbarEl.style.top = `${selectBoxPosition.top - 34}px`;
    }else {
      toolbarEl.style.top = `${selectBoxPosition.top + selectBoxPosition.height - toolbarEl.offsetHeight - 4}px`;
    }
    // 设置图片工具栏的 left
    if (selectBoxPosition.left + selectBoxPosition.width - toolbarEl.offsetWidth >= 0) {
      toolbarEl.style.left = `${selectBoxPosition.left + selectBoxPosition.width - toolbarEl.offsetWidth}px`;
    }else {
