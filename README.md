...
软件的 OCR 识别目前支持 PaddleOCR 和 TesseractOCR 两个离线识别模型和在线的 OCR 识别服务 API。翻译功能需要调用在线的翻译 API。
...
PaddleOCR 是百度开源的 OCR 识别模型，中文的识别准确率比 TesseractOCR 高很多，速度也比 TesseractOCR 更快。

程序已经内置了 PaddleOCR 的识别模型，离线识别，可无限次使用。

PaddleOCR 相比在线的高精度 OCR 识别服务的准确率可能会略差一些，但是用于一般的截图识别已经足够。
...
TesseractOCR 支持使用程序内置的 Tesseract.js，也支持调用设备上安装的 TesseractOCR 软件。
